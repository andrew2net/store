...
//      $model->attributes = $_POST[$this->modelClassName];
      if ($this->saveNode($_POST[$this->modelClassName], $model)) {
        Yii::app()->db->createCommand()->delete('store_category_feature', 'category_id=:id', array(':id' => $model->id));
        if (isset($_POST['feature'])) {
          foreach ($_POST['feature'] as $value) {
            $categoryFeature = new CategoryFeature;
            $categoryFeature->category_id = $model->id;
            $categoryFeature->feature_id = $value;
    ...
    if ($post_node['url'] != $model->url) {
      if (strlen($post_node['url']) > 0) {
        ...
        if (file_exists($uploaded_file)) {
          ...
          $file_name = Yii::getPathOfAlias('webroot') . $img_storage . $model->id . $ext;
...
//        unlink($old_file);
      if (isset($file_name))
        $model->url = $img_storage . basename($file_name);
      else
        $model->url = '';
    }
    else
