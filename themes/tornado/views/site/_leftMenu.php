...
<div>
    <div id="left-menu-cont">
        ...
        $groups = Category::model()->roots()->hasProducts()->order('name')->findAll();
        $items = array();
        $items[] = array('label' => 'Категории:');
        foreach ($groups as $value) {
          /* @var $value Category */
          /* @var $value NestedSetBehavior */
          $groups1 = $value->hasProducts($value->root, 2)->order('name')->findAll();
          $items1 = array();
          foreach ($groups1 as $value1) {
            /* @var $value1 Category */
            /* @var $value1 NestedSetBehavior */
            $groups2 = $value1->hasProducts($value->root, 3)->order('name')->findAll();
            $items2 = array();
